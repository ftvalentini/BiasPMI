"""Concatenate OpenSubtitles archives into a single training corpus.

Obtained from https://github.com/jvparidon/subs2vec/blob/a1a1f5995a6dc0ac5131fba5419100f331765678/subs2vec/clean_subs.py

Our script changes the cleaning part: it does not remove hyphens, apostrophes, slashes or punctuation.
"""
...
def _clean_text(txt, ioformat='txt'):
    """
    """
    if ioformat != 'viz':
...
def join_archive(lang, ioformat='txt', years=(1900, 2050), verbose=False):
    ...
    with open(out_fname, 'w') as outfile:
        for filepath in filepaths:
                outfile.write(_clean_text(read_zip.open(filepath).read().decode('utf-8'), ioformat))
                if verbose:
                    i += 1
